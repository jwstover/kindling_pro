defmodule KindlingPro.Migrator do
  @moduledoc false
  ...
  defmacro __using__(opts) do
    quote location: :keep, bind_quoted: [opts: opts] do
      ...
      @doc false
      def initial_version, do: unquote(opts[:initial_version])

      @doc false
      def current_version, do: unquote(opts[:current_version])
      ...
      def migrated_version(opts) do
        ...
        WHERE pg_class.relname = '#{unquote(table_name)}'
      ...
      defp record_version(%{prefix: prefix}, version) do
        execute("COMMENT ON TABLE #{inspect(prefix)}.#{unquote(table_name)} IS '#{version}'")
      end
